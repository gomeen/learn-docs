...
| # | 类别 | 中文名 | 一句话概括 |
|---|------|--------|-----------|
| A01 | Broken Access Control | 权限控制失效 | 用户能访问/操作本不该访问的资源（[RBAC](../08-authorization/01-rbac.md)） |
| A02 | Cryptographic Failures | 加密失效 | 明文存密码、弱加密、不安全的传输（[加密](../06-encryption/)） |
| A03 | Injection | 注入攻击 | [SQL 注入](./03-sql-injection.md) / NoSQL / OS / LDAP；页面注入见 [XSS](./02-xss.md) |
...
| A07 | Authentication Failures | 认证失效 | 弱密码、会话固定、凭证填充（[认证](../07-authentication/)） |
| A08 | Software & Data Integrity | 软件与数据完整性 | 不安全的反序列化、[CI/CD](../11-cicd/01-concepts.md) 投毒 |
| A09 | Logging & Monitoring Failures | 日志与监控失效 | 无法检测攻击、无法溯源 |
| A10 | SSRF | 服务端请求伪造 | 服务端被迫访问内网资源（详见 [06-ssrf](./06-ssrf.md)） |
...
| A07 认证失效 | Flask-Login + [CSRF](./04-csrf.md) Token | TokenAuthenticationFilter |
