...
- Flask 路由基础（详见 [Flask 基础](../../dify/02-backend/02-flask-basics.md)）
- SQLAlchemy ORM 基础（详见 [SQLAlchemy 映射](../../dify/03-database/02-sqlalchemy-mapping.md)）
...
- 上层依赖下层，**下层不能依赖上层**
- Domain 层定义接口（Protocol，详见 [Protocol 与 Generic](../../dify/01-fundamentals/09-protocol-generic.md)），Repository 层实现接口（依赖倒置；DI 详见 [依赖注入](05-dependency-injection.md)）
...
**解读**：
- 第 7-13 行：多个装饰器叠加完成**横切关注点**（鉴权、权限、租户注入；装饰器原理详见 [装饰器](../../dify/01-fundamentals/10-decorator.md)）
...
- 第 24-26 行：把 ORM 对象转为 Pydantic Response Model 再返回（详见 [Pydantic 基础](../../dify/02-backend/09-pydantic-basics.md)）
...
**解读**：
- 第 7-8 行：业务校验放在 Service 层（权限校验属于业务规则；多租户隔离详见 [多租户架构](../../dify/02-backend/14-multi-tenancy.md)）
