...
事件体常用不可变数据载体表达（`@dataclass` 详见 [dataclass](../../dify/01-fundamentals/22-dataclasses.md)）：
...
- **异步事件**：通过 Celery 任务（见 `api/tasks/`；Celery 架构详见 [Celery 架构](../../dify/04-cache-and-queue/14-celery-architecture.md)）
