...
2. **装饰器注入（Controller 层）**：通过 `@with_current_user`、`@get_app_model` 装饰器注入参数（装饰器原理详见 [装饰器](../../dify/01-fundamentals/10-decorator.md)）
   ```python
   @with_current_user
...
# 定义接口（ABC 详见 [抽象基类 ABC](../../dify/01-fundamentals/21-abc.md)）
...
- `app_id` 等上下文用于多租户隔离（详见 [多租户架构](../../dify/02-backend/14-multi-tenancy.md)）
...
**解读**：
- 第 1-3 行：装饰器签名使用泛型 `T, **P, R`，保证类型安全（详见 [Protocol 与 Generic](../../dify/01-fundamentals/09-protocol-generic.md)）
- 第 13 行：装饰器从 Flask context（`current_account_with_tenant()`）取出当前用户（上下文详见 [Flask 上下文](../../dify/02-backend/03-flask-context.md)）
