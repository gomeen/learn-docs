...
class OrderApplicationService:
    def place_order(self, user_id: str, items: list[LineItem]) -> str:
        ...
        `with self.uow` 是上下文管理器写法（详见 [上下文管理器](../../dify/01-fundamentals/11-context-manager.md)），
...
**应用服务的特征**：
- 一个方法对应一个用例（use case）
- 管理事务（`commit`/`rollback`；SQL 事务语义详见 [事务与隔离级别](../21-sql/04-sql-transaction.md)）
- 协调多个聚合、外部服务
...
3. `Workflow` 聚合根的 `create()` 方法（工厂方法；工厂模式详见 [策略与工厂](../../dify/02-backend/17-strategy-factory.md)，学完后再对照实现）
