...
REST + JSON 是 Web API 主流（REST 见 [REST API 设计](./02-rest-api-design.md)，JSON 见 [JSON 处理](../../dify/01-fundamentals/17-json-processing.md)），但有局限：
...
class ChatServicer(chat_pb2_grpc.ChatServiceServicer):
    ...
    def StreamMessage(self, request, context):
        """服务端流实现：逐 token 输出（`yield` 生成器见 [生成器](../../dify/01-fundamentals/14-generator.md)）。"""
...
**dify 后端主要以 REST API + WebSocket/SSE 为主**（WebSocket 见 [WebSocket](./03-websocket.md)，SSE 见 [SSE](./04-sse.md)）。gRPC 主要出现在以下场景：
