...
| 工具 | 用途 | 在 dify 中的位置 |
|------|------|------------------|
| **Bandit** | Python 静态扫描 | 可作为 pre-commit hook（详见 [Pre-commit Hook](../../dify/09-testing/14-pre-commit.md)） |
...
**核心代码**（async 用法详见 [async/await 与 asyncio](../../dify/01-fundamentals/12-async-asyncio.md)）：
