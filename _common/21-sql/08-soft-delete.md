...
- [3.2.2 主键、外键、唯一约束](07-database-keys.md)
...
常见策略是先从用户视图隐藏，经过保留期再硬删除；或先删主记录，再异步清理大体量从属数据。异步清理必须幂等，并监控失败重试，否则会留下孤儿数据（后台异步任务可参考 [Celery 架构](../../dify/04-cache-and-queue/14-celery-architecture.md)，此处不展开）。
