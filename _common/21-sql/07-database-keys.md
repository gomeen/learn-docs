...
- [3.2.1 三大范式与反范式](06-normalization.md)
...
自然键有业务意义但可能变化；代理键稳定、窄且便于引用。Dify 大量使用 UUID 代理键，并对真正的业务键额外加唯一约束，例如 `(tenant_id, account_id)`（租户内唯一约束详见 [多租户查询](../../dify/03-database/09-multi-tenant-query.md)）。
