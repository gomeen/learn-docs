...
好分片键应高基数、分布均匀，并出现在大部分查询中。多租户系统常以 `tenant_id` 路由，但超级租户可能造成热点（多租户查询过滤详见 [多租户查询](../../dify/03-database/09-multi-tenant-query.md)）。哈希分片均匀，范围分片便于时间清理，但容易出现尾部热点。
...
路由配置可用 `@dataclass` 承载（详见 [dataclass](../../dify/01-fundamentals/22-dataclasses.md)）：
