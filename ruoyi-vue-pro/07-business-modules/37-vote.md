...
ruoyi 通过 Flowable 的 MultiInstanceTask 实现会签/或签，在 BPMN XML 中配置。

### 3.2 自定义候选人

**文件位置**：`/Users/xu/code/github/ruoyi-vue-pro/yudao-module-bpm/`

```java
public class BpmTaskAssignRuleServiceImpl implements BpmTaskAssignRuleService {

    @Override
    public List<Long> calculateTaskCandidateUsers(TaskInfo task) {
        // 1. 读取规则
        BpmTaskAssignRuleDO rule = ruleMapper.selectByTaskDefinitionKey(task.getTaskDefinitionKey());
        // 2. 根据规则计算候选人
        if (rule.getType() == 1) {  // 用户
            return parseUserIds(rule.getOptions());
        } else if (rule.getType() == 2) {  // 角色
            return getUserIdsByRoleIds(parseRoleIds(rule.getOptions()));
        }
        // ... 其他类型
    }
}
...
写出 BPMN XML 的多实例任务配置（并行、3 人会签）。

### 练习 3：挑战（选做）

设计"加权会签"：3 个审批人分别有 30%、30%、40% 权重，总权重达到 60% 即通过。说明如何用 BPMN 实现。

## 6. 参考资料

- `/Users/xu/code/github/ruoyi-vue-pro/yudao-module-bpm/`
- Flowable 多实例：https://www.flowable.com/open-source/docs/
