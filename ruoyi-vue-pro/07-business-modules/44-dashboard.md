  ...
  "components": [
    {
      ...
      "title": "销售趋势",
      "dataSource": {
        "sql": "SELECT date, amount FROM sales_daily"
      }
    },
    {
      "type": "number",
      ...
      "title": "今日订单",
      "dataSource": {
        "sql": "SELECT COUNT(*) FROM trade_order WHERE DATE(create_time) = CURDATE()"
      }
    }
  ]
}
...
阅读 `DashboardServiceImpl.java`，理解数据查询和转换。

### 练习 3：挑战（选做）

设计"双 11 实时大屏"：实时显示销售额、订单数、TOP 商品。说明架构和数据流。

## 6. 参考资料

- `/Users/xu/code/github/ruoyi-vue-pro/yudao-module-report/`
- GoView：https://gitee.com/dromara/GoView
