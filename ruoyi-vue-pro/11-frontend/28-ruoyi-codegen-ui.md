...
虽然本仓库的 vue3 子项目是独立仓库，但代码生成器页面约定结构：
...
| 字段 | 作用 | 示例 |
|------|------|------|
| `formType` | 表单组件 | input、textarea、select、date、upload |
| `dictType` | 字典类型 | sys_user_sex |
| `queryType` | 查询方式 | EQ、LIKE、BETWEEN |
| `htmlType` | 列表显示 | input、image、select |
| `required` | 是否必填 | true / false |
| `listOperation` | 列表操作 | 1=查询、2=新增、4=修改、8=删除 |

### 3.3 与本仓库代码的关联

本仓库 `/Users/xu/code/github/ruoyi-vue-pro/yudao-ui/yudao-ui-admin-vue3/src/api/mes/wm/sn/index.ts` 就是典型的**代码生成器产物**：

```ts
// 文件名、export 命名、API 方法签名都符合生成器约定
...
如果仓库中存在很多相似结构的 API 文件，**几乎都是生成器生成的**，可以基于生成器模板二次修改。
...
```
yudao-spring-boot-starter-codegen/
└── src/main/resources/
    ├── codegen/
    │   ├── vue3/                  # Vue3 模板
    │   │   ├── index.vue.vm       # 列表页模板（Velocity 语法）
    │   │   └── form.vue.vm
    │   └── vue2/                  # Vue2 模板
    └── templates/
```

`.vm` 是 Velocity 模板文件，可以改写模板定制生成内容。
...
阅读本仓库 `/Users/xu/code/github/ruoyi-vue-pro/yudao-ui/yudao-ui-admin-vue3/src/api/mes/wm/productreceipt/index.ts`，列出所有生成器可能用到的字段（formType、queryType 等）。

### 练习 2：进阶

下载 ruoyi 公开仓库 yudao-ui-admin-vue3 的 codegen 页面代码，理解字段配置页的实现。

### 练习 3：挑战（选做）

修改代码生成器模板（`.vm` 文件），让生成的列表页**默认带分页大小 20**，并自动注入"操作列的权限字符串"。
...
- ruoyi 代码生成器文档：https://doc.iocoder.cn/codegen/
