...
**文件位置**：`/Users/xu/code/github/dify/api/core/rag/entities.py`（节选）
**核心代码**：
...
class MetadataFilteringCondition(BaseModel):
    """单个元数据过滤条件"""
    name: str
    comparison_operator: str  # "is", "contains", ">", "<", ...
    value: str | None = None
...
class MetadataFilter(BaseModel):
    ...
    conditions: list[MetadataFilteringCondition]
```

**解读**：
- 用 Pydantic 定义数据结构，自带类型校验
- 支持组合条件：`and` / `or`
- `value` 可选（用于 `is empty` 这类不需要值的操作符）

### 3.2 dify 的元数据 LLM 提取

**文件位置**：`/Users/xu/code/github/dify/api/core/rag/retrieval/template_prompts.py`
**核心代码**（节选）：

```python
METADATA_FILTER_USER_PROMPT_2 = """You have access to the following metadata fields:
{metadata_fields}

The user query is:
{query}

Determine which metadata fields should be used to filter the search results.
For each field, specify the comparison operator and value.
...
**解读**：
- dify 提供多个版本的 prompt 模板，覆盖不同场景
- LLM 把自然语言 → 结构化过滤条件
- 后端拿到 JSON 后执行实际的过滤查询
...
设计一组文档（10 条）的元数据 schema，实现一个支持多条件 AND/OR 过滤的函数。

### 练习 2：进阶

阅读 `template_prompts.py` 中所有 metadata 相关 prompt，理解它们的差异。

### 练习 3：挑战（选做）

实现一个"元数据字段 + 全文检索 + 向量检索"的统一查询语言，支持如 `category=技术 AND year>=2024 AND vector_match(query)`。

## 6. 参考资料

- `/Users/xu/code/github/dify/api/core/rag/entities.py`
