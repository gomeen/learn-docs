...
> 在 [12-async-asyncio](./12-async-asyncio.md) 的基础上，分清三个容易混的词：**协程 / Task / Future**，并会用 `gather`、`create_task` 做并发。
...
- **必读**：[12-async-asyncio](./12-async-asyncio.md)（`async` / `await` / `asyncio.run`）
- 知道「函数可以返回一个对象」即可
...
### 4.1 Celery 的 `AsyncResult` ≈「分布式 Future」

dify 触发工作流时，常把任务丢进 Celery，立刻拿到一个结果句柄（示意）：

```python
from celery.result import AsyncResult

# .delay() 把任务推到队列，立刻返回，不在本进程里算完
task: AsyncResult = execute_workflow_sandbox.delay(task_data_dict)

run_id = task.id          # 类似「订单号」
# status = task.status    # PENDING / SUCCESS / FAILURE ...
# result = task.get()     # 阻塞等待结果（API 层通常不用 get，而是轮询/推送）
```

**和 asyncio.Future 的类比**：

| | asyncio Future/Task | Celery AsyncResult |
|---|---|---|
| 跑在哪 | 当前进程的事件循环 | 另一台/另一个 worker 进程 |
| 立刻返回什么 | Task 对象 | 任务 id / AsyncResult |
| 怎么拿最终结果 | `await task` | 轮询、回调、WebSocket 推送等 |

所以：dify 文档里的「异步工作流」**常常是队列异步**，不都是 `asyncio` 协程。读源码时先分清是哪一种。

### 4.2 异步链路里的错误也要「落盘」

配额不足、派发失败等，不能只在内存里 raise 就完了——需要写日志/状态表，否则事后无法排查。读 `async_workflow_service` 一类代码时，关注：

1. 预留资源（如配额）
2. 派发任务
3. 失败时回滚/记状态，再把异常抛给上层

这和「Task 异常必须有人处理」是同一类工程习惯：**异步不等于可以忽略错误**。
...
写 3 个 `async def job(name, delay)`，内部 `sleep(delay)` 后返回 `name`。

1. 用三个连续 `await` 跑，记录总耗时  
2. 改成 `gather`，再记录总耗时  
3. 用 `as_completed` 打印完成顺序  

### 练习 2：巩固（必做）

故意写一个 `create_task` 后不 `await` 的函数，运行并阅读警告信息；再改成正确版本。

### 练习 3：场景题（推荐）

实现 `safe_call(coro, timeout=1.0)`：超时返回 `{"error": "timeout"}`，正常返回结果。  
再实现 `check_many(names)`：对多个服务做检查，`return_exceptions=True`，输出每个服务 up/down。

### 练习 4：进阶

给练习 1 加上 `return_exceptions=True`：让其中一个 `job` 抛异常，打印 `gather` 的结果列表，确认异常对象在列表里。

### 练习 5：挑战（选做）

用 `asyncio.Semaphore(3)` 限制并发，对 20 个模拟请求做 `gather`，验证同时运行的任务数不超过 3（可在进入/离开 `async with sem` 时打印计数）。  
可选：模拟「用户点停止」——1 秒后 `cancel` 某个长任务并确认抛出 `CancelledError`。
