...
- [1.2.3 环境变量](./29-env-vars.md)
- [1.2.2 配置文件格式](./28-config-file-format.md)
...
dify 的 `DifyConfig` 用 `TomlConfigSettingsSource` 从 TOML 加载（YAML/TOML 语法见 [28-config-file-format](./28-config-file-format.md)），并支持 Apollo、Nacos 远程配置中心：
...
实现一个 `YAMLConfigSettingsSource`，让 `pydantic-settings` 能从 YAML 文件加载配置（YAML 语法见 [28-config-file-format](./28-config-file-format.md)；dify 暂未实现，可以贡献）。
