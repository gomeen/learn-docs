...
### 3.1 注释分层规范：`api/AGENTS.md`

**文件位置**：`/Users/xu/code/github/dify/api/AGENTS.md`
**核心代码**（行 13-27，What to write where 整段）：
...
**解读**：
- 第 13 行：**"Keep notes scoped"** —— 模块注释讲模块级、类注释讲类级、函数注释讲行为契约、段落注释讲局部 why。**禁止重复**。
- 第 15-18 行：模块 docstring 应包含 purpose、boundaries、invariants、gotchas。**链接到协作模块**——降低新成员发现成本。
- 第 17 行：**"Prefer stable facts over ephemeral 'today we…' notes"** —— 注释不应记录临时状态（如"今天我们在重构"），应记录稳定事实（"X 必须是 Y"）。
- 第 19-22 行：类 docstring 应说明 responsibility（职责）、lifecycle（生命周期）、invariants（不变量）、how to use。
- 第 21 行：**stateful 类必须显式记录状态**——否则读者不知道哪个方法会改变状态。
- 第 22 行：**异步/并发假设必须显式记录**——这是 dify 特别强调的，因为异步代码最容易出 race condition。
- 第 24 行：函数 docstring 是**行为契约**——参数、返回、副作用、异常。

### 3.2 注释与代码同步：`api/AGENTS.md` 的 "Rules"
...
阅读 `/Users/xu/code/github/dify/api/AGENTS.md` 第 13-44 行（"What to write where" 和 "Rules"），用自己的话总结 dify 对注释/docstring 的 5 条核心要求。

**参考答案**：见 `solutions/04-docstring.md`

### 练习 2：进阶

打开 `/Users/xu/code/github/dify/api/services/errors/account.py`，阅读所有异常类的 docstring。判断哪些 docstring 是"何时触发"风格、哪些是"做什么"风格。如果混用，请统一为一种风格。

### 练习 3：挑战（选做）

阅读 `/Users/xu/code/github/dify/api/services/async_workflow_service.py`（如存在），找出所有 docstring，分类统计：
- 模块级 docstring 数量
- 类 docstring 数量
- 函数/方法 docstring 数量
- 缺 docstring 的公共函数数量

思考：为什么 dify 这么重视 docstring？这与 AI Agent 的可读性有什么关系？

## 6. 参考资料

- `/Users/xu/code/github/dify/api/AGENTS.md`（注释规范权威定义）
- `/Users/xu/code/github/dify/api/services/errors/__init__.py`（docstring 范本）
- `/Users/xu/code/github/dify/api/services/errors/account.py`（领域异常的 docstring）
